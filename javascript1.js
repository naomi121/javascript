...
for (let j = 0; j < inventory.length; j++) {
    ...
    carYears.push(inventory[j].car_year);
...
for (let k = 0; k < inventory.length; k++) {
    ...
    if (inventory[k].car_year < 2000) {

        oldCars.push(inventory[k].car_model);
...
for (let l = 0; l < inventory.length; l++) {
    ...
    if (inventory[l].car_make === "BMW" || inventory[l].car_make === "Audi") {

        BMWAndAudi.unshift(inventory[l]);
